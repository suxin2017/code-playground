import { getNpmVersionUrl, getFileInfoUrl, getFileUrl, getNpmSearchUrl } from "../common/cdn";
...
export async function loadPackageCdnFiles(libraryName: string, version: string) {
  const packageInfo = await fetchJson<FileInfo>(getFileInfoUrl(libraryName, version));
  return packageInfo;
}
...
export async function loadCDNFile(libraryName:string,version?:string,path?:string) {
